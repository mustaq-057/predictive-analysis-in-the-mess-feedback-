...
# This list contains all the food items we want to include in our reviews
DISHES = [
...
# These are the words we use to describe food in different languages
ADJECTIVES = {
    'telugu': {
...
# These are the sentence structures we use to build reviews
TEMPLATES = {
    'telugu': {
        'good': [
...
# This function adds random typos to make reviews look more real
def add_natural_variation(text):
    # Sometimes convert text to lowercase
    if random.random() < 0.4:
        text = text.lower()
    # Sometimes replace words with common slang
...
# This function creates a list of reviews for a specific language
def generate_reviews(lang, count):
    reviews = []
    # We split the count evenly between good and bad reviews
    good_count = count // 2
    bad_count = count - good_count
    
    # Create the positive reviews
    for _ in range(good_count):
        ...
        # Give a high rating (4 or 5 stars)
    ...
    for _ in range(bad_count):
        ...
        review = template.format(dish=dish, quality=quality)
        review = add_natural_variation(review)
        # Give a low rating (1, 2, or 3 stars)
...
def main():
    print("🚀 Generating Fresh 500k Dataset (100k per language)...")
    
    languages = ['telugu', 'kannada', 'punjabi', 'hindi', 'english']
    all_reviews = []
    
    # Loop through each language and generate 100,000 reviews
    for lang in languages:
        print(f"   Generating {lang.capitalize()}: 100,000 reviews")
        reviews = generate_reviews(lang, 100000)
        all_reviews.extend(reviews)
    
    # Shuffle the reviews so they are mixed up
    ...
    output_path = os.path.join(output_dir, 'fresh_500k_reviews.csv')
